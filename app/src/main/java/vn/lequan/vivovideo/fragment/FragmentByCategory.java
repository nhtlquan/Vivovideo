...
import vn.lequan.vivovideo.adapter.SeriesAdapterCategory;
...
public class FragmentByCategory extends Fragment {
    ...
    private SeriesAdapterCategory seriesAdapterCategory;
    private boolean check = false;
    private ProgressBar marker_progress;

    public FragmentByCategory() {
    }

    public static FragmentByCategory newInstance(int ID) {
        Bundle args = new Bundle();
        args.putInt(OBJECT, ID);
        FragmentByCategory fragment = new FragmentByCategory();
        fragment.setArguments(args);
        return fragment;
    }

    public static FragmentByCategory newInstance() {
        Bundle args = new Bundle();
        FragmentByCategory fragment = new FragmentByCategory();
        fragment.setArguments(args);
        return fragment;
...
//        GlobalApp.getInstance().fragment = FragmentByCategory.newInstance();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        if (view == null) {
            ...
            seriesAdapterCategory = new SeriesAdapterCategory(getFragmentManager(), getActivity(), new ArrayList<ChildCategory>());
